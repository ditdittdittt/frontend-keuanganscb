  ...
  <v-container>
              ...
              <v-card-title class="text-capitalize primary white--text">
                ...
                <v-menu bottom left>
                  ...
                  <v-list>
                    ...
                    <v-list-item
                      @click.stop="
                        $export('excel', 'request', $route.params.id)
                      "
                    >
                      <v-list-item-title class="text-capitalize">
                        {{
    ...
    <v-row v-if="checkStatus()">
      <v-col>
        ...
        >{{ $translate('components.button.already_paid', 'capitalize') }}</v-btn>
      </v-col>
    </v-row>
    ...
    <template>
      <v-row justify="center">
        <v-dialog v-model="dialogSureDelete" persistent max-width="600">
          <v-card>
            <v-card-title class="title text-capitalize">{{ $translate('text.sure_delete_head') }}</v-card-title>
            <v-card-text class="overline">{{ $translate('text.sure_delete_body') }}</v-card-text>
            <v-card-actions>
              <v-row class="mx-0">
